...
int GoogleVoice::SendSMS(string number, string msg)
{
	...
	curl_easy_setopt( hcurl, CURLOPT_POSTFIELDS, data.c_str());	
	// TODO: check this and make sure it doesnt hold on to passed data ptr.
	
	curlbuf.clear(); 
	cr=curl_easy_perform(hcurl);
	if( cr != CURLE_OK ) {
		cout << "curl() error on sending sms: " << errorbuf << endl; 
		return -3;
	}
	...
	regex rexp("\"data\":\\{\"code\":(\\d+)\\}"); 
	cmatch m;
	
	if( regex_search(curlbuf.c_str(), m, rexp) ) {
		string t=m[1]; return atoi( t.c_str() );
	} else {
		cout << "Something went wrong.  Enable debugging.\n"; 
		return -1;
	}
